...
# Left panel: India map and selectors with district detail map
# Right panel: All district/statewise charts, correlation, and pie charts
app.layout = html.Div([
    # CSS for styling including the arrow connector
    html.Div([
        html.Link(
            rel='stylesheet',
    ...
    # --- Left Panel: India Map and District Map with Arrow ---
    html.Div([
        html.Div([
        ...
        # Container for India map and district map with arrow
        html.Div([
            # Main India map (left side)
            html.Div([
                dcc.Graph(id='india-map', style={'height': '82vh', 'width': '100%'})
            ], style={'width': '60%', 'display': 'inline-block', 'position': 'relative', 'verticalAlign': 'top'}),
            
            # Arrow connector (centered between maps)
            html.Div([
                html.Div(style={
                    'width': '50px',
                    'height': '2px',
                    ...
                    html.Div(style={
                        ...
                        'filter': 'drop-shadow(1px 1px 2px rgba(0,123,255,0.2))'
                    })
                ])
            ], style={
                'position': 'absolute',
            ...
            # District detail map card (right side)
            html.Div([
                html.Div([
                    html.H4(id="district-map-title", children="📍 Select a State", 
                           style={
                    ...
                    dcc.Graph(id='district-detail-map', style={'height': '75vh', 'width': '100%', 'margin': '0', 'padding': '5px'})
                ], style={
                    'background': 'white',
                    'borderRadius': '12px',
...
def update_district_detail_map(selected_state, selected_subcat):
    try:
        ...
        # Try to load the state GeoJSON file
        ...
        # Find the correct property key for district names
        ...
        return empty_fig, f"📍 {selected_state.replace('_', ' ').title() if selected_state else 'Error'}"

# =========================
# Arrow and Card Styling Callback
# =========================

@app.callback(
    [Output("map-arrow", "style"), Output("district-map-container", "style")],
    [Input("state-selector", "value"), Input("subcategory-dropdown", "value")]
)
def update_arrow_visibility(selected_state, selected_subcat):
    base_arrow_style = {
        'position': 'absolute',
        'left': '50%',
        'top': '50%',
    ...
    if selected_state and selected_subcat:
        # Show arrow and activate card
        arrow_style = {**base_arrow_style, 'opacity': '1'}
        card_style = {
            **base_card_style,
            'opacity': '1',
            'borderColor': '#007bff',
        ...
        # Hide arrow and deactivate card
        arrow_style = {**base_arrow_style, 'opacity': '0'}
        card_style = {**base_card_style, 'opacity': '0.7'}
    
    return arrow_style, card_style
...
if __name__ == "__main__":
    # Run the Dash app in debug mode on port 8055
